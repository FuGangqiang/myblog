...
{% block main %}
  ...
  <div id="pages">
  {%- if page.index > 1 %}
    <a class="prev" href="{{ tag_pages | get(key=tag.name) | nth(n=page.index - 2) | get(key='name') | urlencode }}">« 上页</a>
  {%- endif -%}
    <span class="spacer"></span>
    <span class="info">{{ page.index }} / {{ tag_pages | get(key=tag.name) | length }}</span>
    <span class="spacer"></span>
  {%- if page.index < tag_pages | get(key=tag.name) | length %}
    <a class="next" href="{{ tag_pages | get(key=tag.name) | nth(n=page.index) | get(key='name') | urlencode }}">下页 »</a>
  {% endif -%}
  </div>
{%- endblock main %}
